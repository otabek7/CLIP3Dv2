...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
  ...
  QObject::connect(&DLP2, SIGNAL(DLPPrintSignal(QString)), this, SLOT(PrintToTerminal(QString)));
  QObject::connect(&DLP2, SIGNAL(DLPError(QString)), this, SLOT(showError(QString)));
  ...
  QObject::connect(&PrintControl2, SIGNAL(ControlPrintSignal(QString)), this, SLOT(PrintToTerminal(QString)));
  QObject::connect(&PrintControl2, SIGNAL(ControlError(QString)), this, SLOT(showError(QString)));
  QObject::connect(&PrintControl2, SIGNAL(GetPositionSignal()), this, SLOT(on_GetPosition_clicked()));

  // Initialize features
  CurrentDateTime = QDateTime::currentDateTime(); // get current time for startup time
  // Initialize settings pointers for DMD1
  ...
  // Initialize settings pointers for DMD2
  ui->SettingsWidgetDMD2->initSettingsPointers(&m_PrintSettingsDMD2, &m_PrintControlsDMD2, &m_PrintScriptDMD2, &m_InjectionSettings);
  PrintControl2.getControlPointers(&m_PrintSettingsDMD2, &m_PrintControlsDMD2, &m_PrintScriptDMD2);

  // Load settings
  loadSettings(); // load settings from settings file

  // Auto connect peripherals
  autoConnect();

  // Initialize settings by updating UI
  initSettings(); // initialize settings by updating UI

  // Initialize plot for DMD1
  ui->GraphicWindow->initPlot(m_PrintControls, m_PrintSettings, m_PrintScript);

  // Initialize plot for DMD2
  ui->GraphicWindow->initPlot(m_PrintControlsDMD2, m_PrintSettingsDMD2, m_PrintScriptDMD2);
}
...
void MainWindow::PrintProcess()
{
  ...
  { // if not at print end
    ...
    if (m_PrintControls.remainingImages <= 0 && m_PrintControls.InitialExposureFlag == false && m_PrintSettings.ProjectionMode != VIDEO)
    {
      QStringList ImageList = GetImageList(m_PrintControls, m_PrintSettings);
    ...
    if (m_PrintSettings.ProjectionMode == VIDEO && (int)m_PrintControls.layerCount < ui->SettingsWidget->FileListCount())
    {
...
{
  if (m_PrintSettings.ProjectionMode == VIDEO && m_PrintControls.FrameCount < ui->SettingsWidget->FileListCount())
  {
  ...
  if (m_PrintSettings.ProjectionMode == VIDEOPATTERN // If in video pattern mode
      && PrintControl.VPFrameUpdate(&m_PrintControls, m_PrintSettings.BitMode, m_PrintSettings.ResyncVP) && m_PrintControls.FrameCount < ui->SettingsWidget->FileListCount())
  {
...
void MainWindow::SetExposureTimer()
{
  ...
  else
  {
    ...
    {                                                     // else grab from settings struct
    ...
    { // if pumping is enabled, go to pumping slot
...
void MainWindow::setScriptParam(float &param, QStringList script, int layerCount)
{
  ...
  { // TODO: verify if the -2 was required for dark time
...
void MainWindow::on_VideoCheckbox_clicked()
{
  if (ui->VideoCheckbox->isChecked())
  {
    ...
    { // light engine connection failed
...
void MainWindow::Check4VideoLock()
{
  ...
  if (LCR_GetStatus(&HWStatus, &SysStatus, &MainStatus) == 0)
  {
    ...
    { // Video source is currently not locked
      ...
      { // System has attempted 15 times to lock to video source
 ...
 * Preps the system for use with iCLIP printer
 */
void MainWindow::on_DICLIPSelectDMD2_clicked()
{
  m_PrintSettings.StageType = STAGE_GCODE; // Set stage to Gcode for the iCLIP printer
  m_PrintSettings.PrinterType = ICLIP;     // Update printer type

  // Enable the injection parameters
  ui->SettingsWidgetDMD2->EnableParameter(CONTINUOUS_INJECTION, ON);
  ui->SettingsWidgetDMD2->EnableParameter(INJECTION_VOLUME, ON);
  ui->SettingsWidgetDMD2->EnableParameter(INJECTION_RATE, ON);
  ui->SettingsWidgetDMD2->EnableParameter(INITIAL_VOLUME, ON);
  ui->SettingsWidgetDMD2->EnableParameter(INJECTION_DELAY, ON);

  // Disable the starting position selection
  ui->SettingsWidgetDMD2->EnableParameter(STARTING_POSITION, OFF);

  // Disable the stage software limits
  ui->SettingsWidgetDMD2->EnableParameter(MAX_END, OFF);
  ui->SettingsWidgetDMD2->EnableParameter(MIN_END, OFF);
}
...
void MainWindow::on_DMD2CLIPSelect_clicked()
{
  m_PrintSettings.StageType = STAGE_SMC;  // Set stage to SMC100CC for the 30 um CLIP printer
  m_PrintSettings.PrinterType = CLIP30UM; // Update printer type

  // Disable the injection parameters
  ui->SettingsWidgetDMD2->EnableParameter(CONTINUOUS_INJECTION, OFF);
  ui->SettingsWidgetDMD2->EnableParameter(INJECTION_VOLUME, OFF);
  ui->SettingsWidgetDMD2->EnableParameter(INJECTION_RATE, OFF);
  ui->SettingsWidgetDMD2->EnableParameter(INITIAL_VOLUME, OFF);
  ui->SettingsWidgetDMD2->EnableParameter(INJECTION_DELAY, OFF);

  // Enable the starting position selection
  ui->SettingsWidgetDMD2->EnableParameter(STARTING_POSITION, ON);

  // Enable the stage software limits
  ui->SettingsWidgetDMD2->EnableParameter(MAX_END, ON);
  ui->SettingsWidgetDMD2->EnableParameter(MIN_END, ON);
}
...
void MainWindow::on_LightEngineConnectButton_clicked()
{
  ...
  if (DLP.InitProjector(target_path1))
...
void MainWindow::on_LightEngineDMD2ConnectButton_clicked()
{
  ...
  if (DLP2.InitProjector(target_path2))
  { // if connection was succesful
...
void MainWindow::saveText()
{
  ...
  QString LogTitle = LogFileDestination + "/" + LogName + "_" + LogDate + "_" + LogTime + ".txt";
...
bool MainWindow::initConfirmationScreen()
{
  ...
  else if (m_PrintSettings.PrinterType == ICLIP)
  {
    ...
    {
      DetailedText += "Pre-Injection Delay: " + QString::number(m_InjectionSettings.InjectionDelayParam) + " ms\n";
    }
    else if (m_InjectionSettings.InjectionDelayFlag == POST)
    {
      DetailedText += "Post-Injection Delay: " + QString::number(m_InjectionSettings.InjectionDelayParam) + " ms\n";
    }
...
void MainWindow::initSettings()
{
  ...
  if (m_PrintSettings.PrinterType == CLIP30UM)
  {
    ...
    ui->CLIPSelectDMD2->setChecked(true);
    ui->DICLIPSelect->setChecked(false);
    ui->DICLIPSelectDMD2->setChecked(false);
    emit(on_CLIPSelect_clicked());
    emit(on_DMD2CLIPSelect_clicked());
  }
  else if (m_PrintSettings.PrinterType == ICLIP)
  {
    ui->DICLIPSelect->setChecked(true);
    ui->CLIPSelect->setChecked(false);
    ui->DICLIPSelectDMD2->setChecked(true);
    ui->CLIPSelectDMD2->setChecked(false);
    emit(on_DICLIPSelect_clicked());
    emit(on_DICLIPSelectDMD2_clicked());
  }
  ...
  ui->SettingsWidgetDMD2->EnableParameter(VP_RESYNC, OFF);     // Always starts in POTF so ResyncVP is disabled from start
  ui->SettingsWidgetDMD2->EnableParameter(INITIAL_DELAY, OFF); // Always starts in POTF so Initial delay is disabled
  ui->SettingsWidgetDMD2->initPrintSettings();

  // Init into video pattern mode
  qDebug() << "proj mode: " << m_PrintSettings.ProjectionMode << m_PrintSettingsDMD2.ProjectionMode << ", connect: " << m_PrintControls.lightConnect << m_PrintControlsDMD2.lightConnect;
  if (m_PrintSettings.ProjectionMode == VIDEOPATTERN && m_PrintControls.lightConnect == ON && m_PrintSettingsDMD2.ProjectionMode == VIDEOPATTERN && m_PrintControlsDMD2.lightConnect == ON)
  {
  ...
  if (m_PrintScript.PrintScript == ON)
  {
    for (uint i = 0; i < m_PrintControls.nSlice; i++)
    {
      ...
      while (!FrameFinished)
      {
        ...
        { // frame finished, add remaining exposure time now with dark time
...
QStringList MainWindow::GetImageList(PrintControls m_PrintControls, PrintSettings m_PrintSettings)
{
  ...
  if (m_PrintControls.InitialExposureFlag == ON && m_PrintSettings.ProjectionMode != VIDEO)
  { // If in initial POTF upload
    m_PrintControls.nSlice = ui->SettingsWidget->FileListCount();
